...
function QuestionField({
  ...
  // Calculer si la question est conditionnellement requise
...
export function MemberRecruitmentForm() {
  ...
  const total = MEMBER_FORM.length;
  const section = MEMBER_FORM[step];
  ...
  if (state.ok) {
    return (
      <div className="qp-card p-10 text-center">
        ...
        <h3 className="qp-title text-2xl text-ink">Candidature envoyée 🤍</h3>
        <p className="mt-3 font-serif text-lg text-ink/80">
          Merci pour ta candidature aux Queen Pearls. Le staff l&apos;étudiera
          avec soin et reviendra vers toi sur Discord.
        </p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="mb-6">
        <div className="flex items-baseline justify-between">
          ...
          <p className="text-xs text-greypearl">{MEMBER_FORM_INTRO.title}</p>
        </div>
      ...
      <div className="mt-8 flex items-center justify-between gap-4 border-t border-or/10 pt-6">
        ...
        {isLast ? (
          ...
          <button type="button" onClick={goNext} className="qp-btn qp-btn--solid">
            Suivant →
          </button>
        )}
      </div>
